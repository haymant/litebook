...
export default {
  ...
  actions: {
    ...
    async login ({ dispatch }, {
      ...
      util.cookies.set('uuid', res.getSessionid())
      util.cookies.set('token', res.getSessionid())
      // 设置 vuex 用户信息
      await dispatch('d2admin/user/set', { name: username, sessionId: res.getSessionid() }, { root: true })
      // 用户登录后从持久化数据加载一系列的设置
      await dispatch('load')
    },
    /**
